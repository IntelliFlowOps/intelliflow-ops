...
import { fetchAllTabs } from '../services/sheetsService.js';
...
export function DataProvider({ children }) {
  ...
  useEffect(() => {
    ...
    autoRefreshRef.current = setInterval(() => { refresh(); }, AUTO_REFRESH_INTERVAL_MS);
  ...
  return (
    <DataContext.Provider value={{ data, loading, initialLoading, refreshing, errors, lastUpdated, isStale, refresh }}>
      {children}
...
export function useTabData(tabKey) {
  const { data, loading, initialLoading, refreshing, errors } = useSheetData();
  return {
    rows: data?.[tabKey] ?? [],
    loading,
    initialLoading,
